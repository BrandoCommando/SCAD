hexd1=12.2; // [hex=12.6,v6=11.8]
hexd2=17; // [hex=16,v6=17]
hexh1=5.7; // grip amount: [hex=4,v6=6]
hexh2=0; // drop amount: [hex=6.4,v6=-1]
mainh=4;
v6h1=6;
v6h2=4.5;
v6d1=12;
...
if(!part)
{
  ...
  rotate([0,0]) intersection(){
    ...
    union(){
      ...
      translate([0,-38,14]) rotate([180,0]) translate([0,-24]) translate(blockmode==3?[0,10,32]:[]) rotate(blockmode==3?[-90,0]:[])
...
{
  translate(center?[-25,-12.5]:[greg?-5.5:0,yoff-(nutside?1.6:0),8]) {
      if(shapetype==0)
...
module hexagon_seat(center=true,greg=false,nutside=false)
{
  ...
  difference(){
    ...
    translate([24.5,12,7.99-hexh2]) {
      cylinder(d=hexd2,h=20,$fn=50);
      if(blockmode>=3&&!nutside)
        translate([hexd2*-.5,0]) cube([hexd2,hexd2/2,20]);
      if(blockmode>=3&&nutside)
        translate([hexd1*-.5,0,-2]) cube([hexd1,hexd1/2,3]);
    }
    translate([24.5,12,0]) cylinder(d2=hexd1,d1=hexd1,h=15.01);
    if(addguide)
  ...
  translate([24.5,12,8-(hexh2+hexh1)]) {
    difference(){
      union(){
        cylinder(d1=od1+odo1,d2=od2+odo2,h=hexh2+hexh1);
        translate([od2*-.5,od2*-.5+(nutside?(trim_back?0:1):(trim_front?trim_front+0.5:1)),0]) cube([od2,od2-2.02,hexh1+hexh2]);
        //translate([9,-10,3]) rotate([-90,0,0]) 
         // cylinder(d=6,h=10,$fn=30);
        
      }
      if(nutside&&blockmode==3)
         translate([hexd1*-.5,0,-.01]) cube([hexd1,hexd1/2,mainh]);
      if(bottom_screws) for(m=[1,-1])
      translate([m*(hexd1/2+2.5),10,hexh1+hexh2-2.5]) rotate([90,0]) {
        if(!nutside)
      ...
      translate([0,0,hexh1]) cylinder(d2=hexd2,d1=hexd2,h=hexh2+.01);
