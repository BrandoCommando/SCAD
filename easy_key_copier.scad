...
//Shape of grip/head
head_grip_type=1; //[0:Circle,1:Square,2:Heart,3:Flying V,4:Stratocaster,5:Les Paul]
...
if (rotate_flat==1){
	rotate([180,0]) chooseKey(key_type);
} else {
  intersection() {
    translate([-50,-10]) cube([200,20,50]);
	translate([30,0,4]) rotate([-90,0,-180]) chooseKey(key_type);
  }
}
...
module createKey_KW1()
{
	...
	notchCutterOffsetHeightZ=0.88;
	notchCutterBaseHeightZ=1;
	notchCutterAngle=45;
...
module writeCustomText(grip_radius,shaft_length,shaft_to_grip,key_width)
{
	...
	scale(1.2)
	{
		writecircle(grip_text,[0,0,key_width+0.5],grip_radius-6, t=key_width);
	}
}
