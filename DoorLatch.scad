part="bolt"; //[bolt,shaft,catch,all]
id=8;
idw=id+.5;
t=2;
t2=3;
...
*translate([0,1.5]) import("DoorLatch_FrontCatch2.stl");
  ...
  difference()
  {
    translate([6,h2*.6,h2*.6]) rotate([45+90,0,0])
    {
      difference()
      {
          ...
          translate([0,0,-.01]) rotate_extrude($fn=40) translate([5,0]) hull() { circle(d=2,$fn=20); translate([-1,0]) circle(d=2,$fn=20); }
//        cylinder(d=12.6,h=2,$fn=50);
      }
    }
    translate([6,-16,0]) rotate([-45,0]) translate([0,23,20]) cylinder(d=12,h=30,$fn=50);
    translate([0,0,-50-boltadj]) rotate([45-boltang,0]) cube(100,center=true);
  }
}
if(part=="catch"||part=="all") {
  intersection(){
    translate([-29.25+(side==2?17.5:-2),-idw/2-co-14.2]) cube([43+(side==0?20:0),idw+co+20,24+bumph]);
...
//        translate([-.5*(id+t*5),-.5*(h3+t*5),0]) cube([id+t*5,h3+t*5,0.1]);
    difference()
    {
      union(){
        //color("blue")
        translate([0,-co/2-t3/2,1.5]) box(d=idw+t*5.5+t3/2+co,h=h3-cdip+.5,w=id+t*6,top=0,bottom=1,r=4);
        if(backbump){
          intersection() {
        ...
        translate([0,idw/2+t*3-.2,h3/2]) rotate([90,0,0]) box(w=id+cmho*2+t*14,d=h3+4,h=5,top=1,r=1);
        for(m=[0,1]) mirror([m,0])
        translate([-1*id-t-1,idw/2-3.5,0]) difference(){
          union(){
          cube([4,5,h3-cdip+2]);
      ...
      translate([0,-co+t/4+.7,3]) box(d=idw+t*4+t3+.4,h=h3+2.02,w=id+t*3+1.8,r=2.5,bottom=1);
      //holes:
      cht=[15,-15];
      if(centerhole)
        translate([0,14,h3/2+1]) rotate([90,0]) {
          translate([0,0,10]) cylinder(d=7,h=20,$fn=40);
      ...
      translate([0,0,1]) {
        for(x=(side==1?[-1*id-cmho]:(side==0?[-1*id-cmho,id+cmho]:(side==2?[id+cmho]:[]))))
      translate([x,idw+1,h3/2]) rotate([90,0,0]) union() { translate([0,0,-10]) cylinder(d=4,h=20); translate([0,0,4]) cylinder(d1=4,d2=9,h=2); translate([0,0,5.99]) cylinder(d=9,h=20); }
    }
    }
  }
  }
