...
part="rail3"; // [all:Preview Tank,all2:Preview New Tank,wheel:Front Wheel,wheel_back:Back Wheel,wheel_pulley:Wheel Pulley,tread:Single Tread Piece,track:Full Track of Tread (3x8),adapter:Stock Motor to Wheel Adapter,adapter2:Small Stepper Pulley,rail:Inside Rail,rail_outside:Outside Rail,rail2:New Rail,rail2_outside:New Outside Rail,frame:Tank Frame,skid:Skid Plate,bumper:Front Bumper,battery_pack:Battery Pack]
mid_width=110;
tread_height=18;
tread_width=34;
tread_hole_offset=0;
...
module battery_pack(which="body")
{
  ...
  if(which=="body") {
    ...
    difference(){
      union(){
        intersection() {
          translate([-25,-25]) cube([50,50,4]);
          cylinder(d=50,h=3.29,$fn=100);
        }
        translate([0,0,-8]) cylinder(d=28,h=8,$fn=100);
      }
      translate([0,0,-8.01]) cylinder(d=22.2,h=8.02,$fn=90);
      rotate([0,0,45])
      mirrory()
      translate([-25,21.5,-.01]) cube([50,10,ph+.02]);
      rotate([0,0,45])
      mirrorx()
      translate([21.5,-25,-.01]) cube([10,50,ph+.02]);
      translate([0,0,-1]) cylinder(d=18,h=6,$fn=30);
    ...
    difference(){
      intersection() {
        translate([-25,-25,3.3]) cube([50,50,4]);
...
module wheel_edge()
{
  ...
  difference() {
    ...
    translate([0,0,-.01]) cylinder(d=22.2,h=6.02,$fn=60);
    for(r=[0:45:359]) rotate([0,0,r]) translate([18,0,-.01]) {
      ...
      cylinder(d=3,h=6.02,$fn=20);
    }
  }
}
module wheel_spikes()
{
  bd=wheel_track_diameter/2*9.68; //48.4
  difference() {
    union() {
...
module wheel_pulley()
{
  ...
  ts=str(teeth);
  bd=9.68*wheel_track_diameter/2;
  difference(){
    union() {
      *cylinder(d=bd,h=2,$fn=100);
      translate([0,0,2]) pulley("GT2 2mm" , tooth_spacing (2,0.254,teeth=teeth) , 0.764 , 1.494, 12, idler = 0, retainer = 0, motor_shaft = 3.2, draw_base = 0, teeth=teeth );
    }
    if(teeth>70)
    rotate([0,0,22]) translate([0,0,7.5]) linear_extrude(2,convexity=10) text(str(ts[0],"        ",ts[1]),size=9,valign="center",halign="center");
...
module tank2()
{
  ...
  *mirrorx() {
  ...
  translate([mid_width/2+28.5,tread_height/-2,fd/-2-3]) {
    for(y=[tread_height:tread_height:dist]) {
      translate([0,y]) tread2();
      translate([0,dist-y+tread_height,fd+6]) rotate([180,0]) tread2();
    }
    }
  for(r=[-180:360/wheel_track_diameter:0])
  rotate([r,0]) translate([mid_width/2+28.5,tread_height/-2,fd/-2-3]) tread2();
;
  translate([0,dist]) for(r=[-180:360/wheel_track_diameter:0])
  rotate([r+180,0]) translate([mid_width/2+28.5,tread_height/-2,fd/-2-3]) tread2();
  }
  ...
  difference() {
    union() {
      for(x=full?[0,dist]:[0]) {
        ...
        translate([x,0,thick-.01]) cylinder(d1=14,d2=12,h=x>0?5:2,$fn=30);
        if(top_shelf)
...
module rail3(inside=1,right=1,color=1)
{
  thick=6.5;
  full=1;
  top_shelf=0;
  fd=4.84*wheel_track_diameter+1.6;
  echo(str("Rail 3 FD: ",fd));
  difference() {
    union() {
      for(x=full?[0,dist]:[0]) {
        color(color?"blue":undef) translate([x,0]) cylinder(d=full?fd:20,h=thick,$fn=100);
    ...
    for(x=full?[0,dist]:[0])
      translate([x,0,4]) difference() {
        cylinder(d=full?fd-4:16,h=thick-3.99,$fn=100);
        translate([0,0,-.01]) cylinder(d1=14,d2=12,h=2,$fn=30);
      }
    for(x=[0,dist],y=[0,40])
      translate([x,y,-1]) cylinder(d=8.4,h=thick+15,$fn=30);
    //for(m=full?[0,1]:[0]) mirror([m,0]) translate(m?[dist*-1,0]:[])
    ...
    for(x=[39])
    {
      ...
      //translate([x,0])
        translate([31,31]) cylinder(d=3,h=thick+2,$fn=20);
        translate([0,0,1.3]) linear_extrude(thick+2) for(r=[-8:5]) hull() {
          rotate([0,0,r]) translate([31,31]) circle(d=3,h=thick+2,$fn=20);
          rotate([0,0,r+1]) translate([31,31]) circle(d=3,h=thick+2,$fn=20);
        }
        linear_extrude(thick+2) for(r=[-8:5]) hull() {
          rotate([0,0,r]) 
...
;
  difference() {
    union(){
      ...
      translate([0,0,34]) mirror([0,0,1]) color("green") wheel_pulley();
...
module tread2(rd=7,mh=18,mw=34)
{
  segh = mh*2/3-1;
    ...
    if(neg)
    {
      for(m=[0,1]) translate([0,0,m?65+noff:0]) mirror([0,0,m]) {
        ...
        translate([0,0,-.71]) {
          ...
          *translate([-5,-4.01]) cube([10,4,2.02]);
        }
