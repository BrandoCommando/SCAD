...
if ( profile == 12 ) { mirror([0,0,1]) difference() {
    pulley ( "GT2 2mm" , tooth_spacing (2,0.254,teeth) , 0.764 , 1.494 ); 
    translate([0,0,-1]) cylinder(d=3.2,h=20,$fn=20);
}
  }
...
module pulley( belt_type , pulley_OD , tooth_depth , tooth_width, profile = profile, teeth = teeth, idler = idler, motor_shaft = motor_shaft, draw_base = 1, retainer = retainer )
	{
...
//	************************************************************************
	...
	translate ([0,0, pulley_b_ht + pulley_t_ht + (retainer?retainer_ht:0) ]) rotate ([0,180,0])
