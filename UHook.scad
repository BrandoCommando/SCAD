...
// Pre-defined output
part=3; // [0=Custom,1=Door Towel Hook,2=Door Towel Hook X5,3=Small Conduit Hook] */

// (Custom only) Choose between classic hook with screw-holes (0) or "bracket" system (1)
bracket=0; // [0, 1]

// (Custom only) Choose between square (0) and round bracket (1)
bracket_round=0; // [0, 1]

// (Custom only) Choose to have a second hook (1) or not (0)
second_hook=0; // [0, 1]

// (Custom only) How many small hooks
second_hook_count = 4; // [0:8]
...
thickness=15; // [0.1:250]



/* [SECOND HOOK] */

// length of second hook. Good values are around "Hook Size" x 0.75
second_hook_length=30; // [0.1:200]

// Angle of second hook. Best is between 45° and 65°.
second_hook_angle=45; // [0:90]
...
// Bracket's main size
bracket_size=35; // [0:250]

// Bracket's back length
stop_length=15; // [0:250]

// Bracket Stiffness (only with square bracket)
bracket_stiffness=3; // [0:100]
...
ECHO : TOTAL_SIZE_ON_Y_AXIS = y (total length : adjust with "spacer_1/2/3/4")
...
if(part==0) {
  ...
  hook_size=25;
  thickness=15;
  second_hook_length=hook_size*.9;
  second_hook_angle=55;
  spacer_1=10;
  spacer_2=10;
  spacer_3=5;
  spacer_4=5;
  ...
  echo("Making Door Towel Hook");
  main();
} else if(part==2) {
  echo("Making Door Towel Hook X5");
  bracket=1;
  bracket_round=0;
  second_hook=1;
  second_hook_count = 5;
  ...
  difference() {
    main();
    for(y=[45,-55])
      translate([.01,y,thickness/2]) rotate([0,-90]) union(){
        cylinder(d=screw_diam,h=20,$fn=30);
        cylinder(d1=8,d2=screw_diam,h=2,$fn=30);
      }
  }
} else {
  echo("UNKNOWN PART");
...
if (second_hook)
{
    echo (SECOND_HOOK_length=second_hook_length);
...
module uhook ()
{
    union ()
    {
      ...
      if(second_hook_count<=1)
      {
        ...
        if(second_hook_count>1)
        translate([-width_global/2,-height_bottom/2-stiffness,thickness/-2])
        {
          for(i=[1:second_hook_count-1])
            translate([0,height_bottom*(2-i)-stiffness])
              arm();
        }
...
module arm ()
{
    ...
    union ()
    {
        cube ([stiffness, height_bottom, thickness]);
        ...
        union ()
        {
            ...
            union ()
            {
                hull ()
                {
                    cube ([stiffness, second_hook_length, thickness]);
                    rotate ([0,0,second_hook_angle])
                    translate ([0,-stiffness,0])
                ...
                rotate ([0,90,0])
                translate ([-thickness/2,second_hook_length,stiffness/2])
                tip ();
            }
        }
    }
        ...
        if (safety_screw)
        {
            ...
            union ()
            {
                ...
                difference ()
                {
                    // stop main cube
                    color ("DeepSkyBlue")
                    translate ([0,-stop_length/2-bracket_stiffness/2,0])
                    cube ([bracket_stiffness,stop_length,thickness], center=true);

                    // rounding stop's bottom extremity
                    if (stop_length>=bracket_stiffness)
                    translate ([0,-stop_length,0])
                    difference ()
                    {
        ...
        else
        {
            union ()
            {
                ...
                difference ()
                {
                    color ("DeepSkyBlue")
                    // stop main cube
                    translate ([-bracket_size/2-bracket_stiffness/2,-stop_length/2-bracket_stiffness/2,0])
                    cube ([bracket_stiffness,stop_length,thickness], center=true);

                    // rounding stop's bottom extremity
                    if (stop_length>=bracket_stiffness)
                    translate ([-bracket_size/2-bracket_stiffness/2,-stop_length,0])
                    difference ()
                    {
                        cube ([bracket_stiffness+0.1,bracket_stiffness+0.1,thickness+0.1],center=true);
                        translate ([bracket_stiffness/2,bracket_stiffness/2,0])
...
union ()
{
    ...
    hull ()
    {
        ...
        translate ([-bracket_round_diam/2+stiffness/2,-stop_length+stiffness/2,0])
        cylinder (d=stiffness, h=thickness, center=true);
    }
...
if ((bracket)&&(safety_screw)&&(bracket_round!=true))
{
    ...
    if (stop_length<bracket_stiffness)
    echo (" WARNING : Stop length should be bigger than Bracket Stiffness ");
}
...
if (second_hook)
{
    if (second_hook_length>hook_size)
    echo (" WARNING : Second Hook length should be smaller than Hook Size ");
    if (second_hook_length<hook_size/3)
    echo (" WARNING : Second Hook length seems very small compare to Hook Size ");
