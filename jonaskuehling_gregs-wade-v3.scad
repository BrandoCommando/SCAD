...
*/
			...
			for (mount=[0:1]){
				translate([-filament_feed_hole_offset+mount_hole_offset*((mount<1)?1:-1),
...
//				rotate(360/16)
          ...
          if(!mount)
          translate([0,0,base_thickness+1.01]) cylinder(d1=13,d2=2,h=20,$fn=12);
        }
...
module groovemount_holes (){
  ...
  cylinder(d=10,h=extruder_recess_h+10);
  
  cylinder(d=4.4,h=extruder_recess_h+50);
}
