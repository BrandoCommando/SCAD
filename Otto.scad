// Part to print
part="body"; // [all:Preview Bot,head:Head,body:Body,leg:Leg,foot_left:Left Foot,foot_right:Right Foot]
...
// Servo Type for hip
servo_type_hip="mg90s"; // [sg90:SG90 (nylon gear micro servo),mg90s:MG90S (metal gear micro servo)]
...
head_screwless=2; // [1:Snap-fit,2:3mm Screws]
head_screw_nut=0; // [0:No Nuts,1:3mm Hex Nuts,2:3mm Square Nuts]
...
body_switch=2; // [0:None,1:8mm Toggle,2:4x9 Slider]
...
//    translate([left+1.75,botm+1.75,2]) box_chamfer([dims[0]-3.5,dims[1]-3.5,dims[2]],ch=1,chvx=4,chvy=4,inside=1);
    ...
    if(head_screwless==1)
      mirrory() translate([-11.7,right-body_thickness-.51,dims[2]-11]) {
        cube([22.35,body_thickness+0.6,4]);
        translate([0,body_thickness+.02,4.1]) rotate([-35,0]) mirror([0,0,1]) mirror([0,1,0]) cube([22.35,4,5.4]);
      }
    else if(head_screwless==2)
      rotate([0,0,90]) mirrory() mirrorx() translate([-left-2,botm+10.5,dims[2.0]])
        translate([-.01,6.5,-7]) rotate([0,90,0]) cylinder(d=3,h=2.02,$fn=20);
  ...
  translate(move?[0,0,body_height+head_height-3]:[]) rotate(move?[180,0,-90]:[]) {
    ...
    difference(){
      ...
      translate([-5.6,-29.555,3.75]) {
        translate([11.25,-.9,-1]) rotate([-90,0]) rotate([0,0,180]) intersection() {
    ...
    mirrorx() translate([-left-2,-9.25,dims[2]]) rotate([-90,0]) mirror([0,1,0]) linear_extrude(21,convexity=10) polygon([[0,0],[0,8.4],[1.98,8.4],[4.1,6.3],[4.1,4.2],[2,2.5],[2,0],]);
      ...
      mirrory() mirrorx() translate([-left-2,botm+10.5,dims[2.0]]) difference() {
        union() {
        cube([2,10,4]);
        translate([-2,5,4]) rotate([0,90]) {
          if(head_screw_nut>0)
          translate([0,0,1]) cylinder(d1=8,d2=10,h=1,$fn=40);
          translate([0,0,2]) cylinder(d=10,h=2,$fn=40);
        }
          
        }
        translate([-2.01,5,4]) rotate([0,90,0]) {
          cylinder(d=3,h=4.02,$fn=20);
          if(head_screw_nut==1)
            cylinder(d=7.1,h=3,$fn=6);
          else if(head_screw_nut==2)
...
module foot(move=0) {
  ...
  difference(){
    ...
    if(idler_screw)
      translate([15.8,-9.5,15.75]) rotate([0,90,0]) cylinder(d=3,h=10,$fn=20);
