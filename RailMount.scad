...
pipe_diameter = 30; //inches(1.163); //inches(1.163); //30; //inches(1);
...
if(part=="preview") {
  ...
  *yplate();
  ...
  translate([rod_dist*-.5,-30.2,86+zlift]) {
    ...
    for(x=[0,rod_dist]) translate([x,0]) {
      if(x==0) mirrory(rod_length-22) translate([-16,-10,46]) {
        translate([0,4,2]) xstop(1);
        xstop(2);
        translate([0,20]) mirror([0,1]) xstop(3);
        translate([-21,0,16]) xstoptop();
      }
  ...
  !translate([-10,0,4]) rotate([0,0]) intersection(){
    translate([-30,-10,-4.59]) cube([50,80,30]);
    union() {    
      ystop(2,1);
      *ystop(3);
  ...
  translate([30,26,-4.5]) rotate([180,0]) difference() {
    ystop(2);
  ...
  mirror([0,0,1]) {
    ...
    *translate([0,60,5]) mirror([0,1])  mirror([0,0,1]) rotate([-90,0]) translate([0,-25,-4]) xstop(3);
...
module ystop(which=3,guides=0)
{
  ...
  rotate([0,0,0]) difference() {
    union() {
      ...
      if(which!=1)
        translate([-19-3.5,-7,3]) mirror([0,0,1]) rcube([19+10,48-2,30]);
      if(guides)
    ...
    if(which!=1)
    {
      translate([-8-base_thick/2-.5,-8.01,-pipe_diameter+1.99]) cube([base_thick+1.02,50.02,10]);
      for(y=[-2,20]) translate([-8,y,-pipe_diameter+11.98]) {
        hull() {
        ...
        hull() {
          for(x=guides==1?[-10:10]:[0]) translate([x,0,0]) {
            //cylinder(d=7,h=5,$fn=30);
            translate([-2.95,-2.95]) cube([5.9,5.9,10]);
          }
        }
        hull() {
          for(x=guides==1?[-10:10]:[0]) translate([x,0]) cylinder(d=3.2,h=pipe_diameter-7.96,$fn=20);
          }
      }
      for(y=[8,30]) translate([-8,y,-pipe_diameter+11.98-8.6]) {
        hull() {
          for(x=[0]) translate([x,0]) cylinder(d=4,h=pipe_diameter-7.96,$fn=20);
        }
        hull() {
          for(x=[0]) translate([x,0,pipe_diameter-13.96]) cylinder(d1=3.5,d2=10,h=2.02,$fn=40);
          }
        hull() {
          for(x=[0]) translate([x,0,pipe_diameter-11.97]) cylinder(d=10,h=4,$fn=40);
          }
      }
    }
  }
...
module yplate_2d(wheel_space=wheel_dist)
{
  ...
  *import("LowRider2_CNC_-_Full_Sheet_Capable_CNC_Router-_25.4mm/files/Y_plate.DXF");
  difference() {
    mirrorx() {
      if(yplate_version==2)
        translate([0,80]) offset(10) polygon([[0,0],[maxx+10,0],[maxx,10],[maxx,maxy-20],[maxx-10,maxy],[50,maxy],[40,maxy-20],[0,maxy-20]]);//square([200,160]);
      hull() {
    ...
    mirrorx() {
      ...
      if(yplate_version==2)
      {
        for(x=[60,80]) translate([x,maxy+80]) circle(d=5,$fn=20);
        hull() {
          translate([70,120]) circle(d=30,$fn=60);
          translate([70,230]) circle(d=30,$fn=60);
        }
        for(z=[60,220])
    ...
    if(yplate_version==2)
    {
      translate([-10,112]) rsquare([20,inches(4)+6],r=3.4);
      translate([-40,240]) rsquare([80,20],r=10);
    }
  }
}
module gt2_track()
...
module xstop(which=1,mod=0)
{
  ...
  difference() {
   union() {
      if(which==3)
      {
        translate([-24,5,4]) cube([32,20,25.5]);
        translate([-21,-4,4]) cube([57,29,2]);
      }
      else if(which==1)
        translate([-21,-4,2]) cube([26,25,15]);
      else if(which==2)
        translate([mod?-5:-21,-4,-8]) cube([57-(mod?16:0),29,12+(mod?2:0)]);
      
    }
    if(which==3)
    {
      for(z=nutz) translate([-8,26,4+z]) rotate([90,0]) cylinder(d=nutd+.4,h=6,$fn=20);
      translate([-21.5,4.9,6]) cube([27,18,20.5]);
    ...
    if(which!=1)
      for(x=mod?[16]:[12,26],y=[5,16]) translate([x,y,-8.1]) {
        cylinder(d=3,h=20,$fn=20);
        if(mod)
    ...
    if(which==2) {
      translate([0,0.25,1]) mirror([0,0,1]) {
        cube([31,20.5,60]);
...
module xmount_2d() {
  difference() {
      hull() {
        ...
        mirrorx() {
          translate([18,-32]) circle(d=20,$fn=50);
          translate([28,23.4]) square([2,4]);
        }
      }
      translate([0,-70]) circle(d=25,$fn=50);
...
module idler_bearing() {
  difference() {
    union(){
      ...
      translate([0,0,1]) cylinder(d=18,h=10,$fn=80);
      translate([0,0,11]) cylinder(d1=18,d2=20,h=1,$fn=80);
    }
    translate([0,0,-.1]) cylinder(d=14,h=20,$fn=50);
    translate([0,0,1]) cylinder(d=16,h=10,$fn=50);
    translate([0,0,10.9]) cylinder(d=16,h=2,$fn=50);
  }
}
module xmount()
{
...
module yplate()
{
  ...
  /**/
    mirrorx()
    translate([-154.1+xo,-49-wheel_thick,83]) rotate([-90,0]) difference() {
...
module router_plate_2d(vac=0,which=1,cutout=1,dw660=0,makita=1)
{
  difference() {
    union(){
      ...
      intersection() {
        if(which==2)
          ...
          translate([49,plate_size[1]/2-40]) rsquare([80,80]);
        else if(which==4||which==5)
          translate([55,plate_size[1]/2-34]) rsquare([68,68]);
        union() {
          rsquare(plate_size,slide_close);
          mirrory(plate_size[1]) translate([plate_size[0]/2-35,-10]) rsquare([70,25]);
        }
      }
      if(which==3)
    ...
    *for(x=[-20,-32],y=[-8,0,8]) translate([x,y]) circle(d=dw660?5:(which==3?6.2:3),$fn=which==3&&!dw660?6:20);
    ...
    translate(plate_size/2) {
      ...
      *circle(d=50,$fn=80);
      ...
      //if(which>=3)
      mirrorx() mirrory() translate([35,35]) circle(d=4,$fn=30);
    }
