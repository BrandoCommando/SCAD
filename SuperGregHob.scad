...
////////// RENDER IDLER /////////////////////////////////////////////////////////////////////
*translate([78+extra_gear_separation,-10,16.20-(1.5-filament_diameter/2)])		// POSITIONING
	rotate([0,-90,0])
		wadeidler();
...
mirror([0,0]) {
    translate(motor_mount_translation){
      ...
      translate([-2,0]) cube([4,13,6.5]);
      translate([-13,-2]) cube([13,4,6.5]);
      cylinder(d=20,h=6.5,$fn=50);
    }
  }

  difference() {
    wade(hotend_mount=groovemount, legacy_mount=false);
    translate([-30,-.01,wade_block_depth/2-10.4]) cube([80,3,120.8]);
  }
}
...
module wade (hotend_mount=0,legacy_mount=false){
	difference (){
		union(){
			// The wade block.
			cube([wade_block_width,wade_block_height-10,wade_block_depth]);
			...
			cube([wade_block_width+extra_gear_separation,
				wade_block_height-motor_mount_translation[1]+hole_for_608/2-10,
		  ...
		  *linear_extrude(height=motor_mount_thickness)
			...
			difference(){
				...
				translate([-block_bevel_r,block_bevel_r+base_thickness,-.01])
				cylinder(r=block_bevel_r,h=wade_block_depth+.02,$fn=60);
				translate([wade_block_width+block_bevel_r,
					block_bevel_r+base_thickness,-.01])
				cylinder(r=block_bevel_r,h=wade_block_depth+.02,$fn=60);
			}
...
module block_holes(legacy_mount=false){
	...
	*translate(idler_fulcrum)
	...
	translate(motor_mount_translation){
      ...
      {
        b608(h=8);
        translate([0,0,7.99]) cylinder(d1=12,d2=10,h=1.02,$fn=30);
      }
      cylinder(d=20,h=6.6,$fn=50);
    }
		
			translate([0,0,wade_block_depth-8])
			b608(h=9);
...
//				rotate(360/16)
				{
          cylinder(d=m3_diameter+.4,h=base_thickness+2,$fn=20);	
          if(!mount&&off<20)
          {
          translate([0,0,base_thickness+1.01]) difference() {
              hull() {
                translate([0,0,20]) sphere(d=2,$fn=12);
                cylinder(d1=13,d2=2,h=20,$fn=12);
                translate([20,0]) cylinder(d=30,h=10,$fn=40);
              }
              translate([8,0]) cube(20);
            }
          }
        }
...
//				cylinder(r=m4_nut_diameter/2,h=base_thickness,$fn=6);
          ...
          cylinder(d=8,h=base_thickness,$fn=30);
          *rotate([0,0,30]) nut_trap(m4_wrench,base_thickness,0);}
        }
			}
