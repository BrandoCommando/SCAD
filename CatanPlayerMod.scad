difference()
{
  *import("matterhorn10k.stl");
  translate([-51,-51,-30]) cube([102,102,62]);
  translate([0,0,12]) cylinder(r1=20,r2=10,h=20,$fn=6);
}
/*
...
for(f=[1,2.6,13.6]) translate([0,0,f]) rotate([0,0,45]) {
  ...
  translate([-2.125,-2.125,14.8]) difference() {
    cube([4.25,4.25,2.8]);
    translate([0.2,0.2,2.6]) cube([3.85,3.85,2]);
    translate([2.125,2.125,2.4]) linear_extrude(2) {
      difference(){
        circle(d=3,$fn=20);
        circle(d=2.8,$fn=20);
      }
