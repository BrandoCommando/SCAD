...
// ===============================================================================
// box parameters
width       =       20;       // width of the chainlink (Standard: 16.6)
height      =       13;         // height of the chainlink (Standard: 13)
length      =       30;         // length of the chainlink  (Standard: 25, min 30 für Eckig)


// ===============================================================================
// === OUTPUT
if(!part) output_cablechain();

// ===Check Consistency of Values
module output_cablechain(ChainDef=ChainDef) {
translate(len(ChainDef)==1?[-10.05,-33.75]:[])
...
difference()
    ...
    union()
        {
        translate([1.4+exclearance,2+exclearance,0]) cube([4,width-3.7-2*exclearance,3]);
        difference()
            {
                union()
                   {
                ...
                union()
                    ...
                    union()
                        ...
                        translate([(height/13)*4.6,2+exclearance,(height/13)*6.5])
                            rotate(a=90, v=[1,0,0])   
                        ...
                        //FEMALE
                         ...
                         translate([(height/13)*4.6,width+1.3-exclearance,(height/13)*6.5])
                            rotate(a=90, v=[1,0,0])   
                                cylinder(r=(height/13)*6.85, h=3);
                        }
                    difference()
                        ...
                        {
                        translate([(height/13)*5-.0,width+0.4,(height/13)*5.5])
                            rotate(a=90, v=[1,0,0])   
    ...
    union()
        {
            translate([(height/13)*-1,3.4,1.5])
                rotate(a=0, v=[0,0,1])
                    union() {
                      tx=(height/13)*(length+1);
                      ty=width-6.6-exclearance*2;
                      th=height-3;
                      translate([0,exclearance]) {
                      cube([tx,ty,th-ty/2]);
                      translate([0,ty/2,th-ty/2-.01]) rotate([0,90,0]) linear_extrude(tx) rotate([0,0,90]) intersection() { circle(d=ty); translate([ty*-.5,0]) square([ty,ty]); }
                    }
                  }
           translate([(height/13)*(length-9.65),3.4,(height/13)*3.5])
            ...
            if (Type == 2)
            {
            translate([(height/13)*-1,3.4+exclearance,3])
                    rotate(a=0, v=[0,0,1])
                        cube([(height/13)*8,width-6.6-exclearance*2,(height/13)*16]);
            translate([(height/13)*(length-12.5),1.6,1.5])
                    rotate(a=0, v=[0,0,1])
                        cube([(height/13)*6,width-3,height]);
