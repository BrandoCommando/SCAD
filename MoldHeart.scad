...
difference() {
  translate([0,yoff,rd/2]) {
    mykowski(height=h-8,curve=rd/2)
    shape_profile();
    for(z=[0:10])
      translate([0,0,rd/-2+z]) linear_extrude(1.01) shape_profile(id=id);
  }
  if(len(text)>0)
...
module mykowski(height=10,curve=2,max=100) {
  if(curve<=100) {
    minkowski() {
      linear_extrude(height,convexity=4) children(0);
      *rotate([45,45]) cube([curve,curve,curve]);
      sphere(d=curve*2,$fn=40);
    }
  } else {
      *for(z=[-1*curve:0])
      {
        #translate([0,0,z]) linear_extrude(1.1,convexity=4) scale(max*(abs(z)/curve)/100) children(0);
      }
      linear_extrude(height,convexity=4) children(0);
  }
}
